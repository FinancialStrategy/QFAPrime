...
class ProfessionalConfig:
    ...
    max_assets: int = 30
    selected_universe: str = "global_multi_asset"

    # Reporting / chart settings
    ...
    # Optional extensibility buckets
    ...
    def __post_init__(self) -> None:
        # Keep these two synchronized for downstream modules
        if self.trading_days <= 0:
            self.trading_days = 252
        self.annual_trading_days = int(self.trading_days)

        # Ensure confidence_level is included in confidence_levels
        cleaned_levels = []
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.default_symbols, list):
            raise TypeError("default_symbols must be a list of strings.")

        if not isinstance(self.benchmark_map, dict):
            raise TypeError("benchmark_map must be a dictionary.")

        if not isinstance(self.scenario_families, list):
            raise TypeError("scenario_families must be a list of strings.")
